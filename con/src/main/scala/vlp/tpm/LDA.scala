...
class LDA(val sparkContext: SparkContext, val config: ConfigLDA) {
  ...
  def createDataset(path: String, format: String = "json"): Dataset[Document] = {
    val sparkSession = SparkSession.getActiveSession.get
    val data = if (format == "json") sparkSession.read.json(path).as[Document]
      else sparkSession.read.text(path).withColumnRenamed("value", "text").as[Document]
    if (config.verbose)
      data.show()
    data
  }
  ...
  def train(dataset: Dataset[Document]): PipelineModel = {
    ...
    dataset.show(true)
    // create pipeline
    val tokenizer = new RegexTokenizer().setInputCol("text").setOutputCol("tokens").setMinTokenLength(2).setPattern("""[\s.;"',/\-)(?:!]+""")
