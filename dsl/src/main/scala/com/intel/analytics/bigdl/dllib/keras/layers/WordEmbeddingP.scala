 ...
 */
class WordEmbeddingP[T: ClassTag] private(
...
object WordEmbeddingP {
  ...
  def apply[@specialized(Float, Double) T: ClassTag](
      ...
      inputLength: Int = -1)(implicit ev: TensorNumeric[T]): WordEmbeddingP[T] = {
    ...
    id.getAndIncrement()
    new WordEmbeddingP[T](inputDim, outputDim,
      new WordEmbedding.EmbeddingMatrixHolder[T](embeddingMatrix, "WordEmbedding" + id.toString),
      trainable, shape)
  }
  ...
  def getOutputDimFromEmbeddingFile(embeddingFile: String): Int = {
    embeddingFile.split("/").last match {
      ...
      case "numberbatch-EN-19.08.vocab.txt" => 300 // phuonglh
      case "numberbatch-ES-19.08.vocab.txt" => 300 // phuonglh
      case "numberbatch-FR-19.08.vocab.txt" => 300 // phuonglh
      case "numberbatch-PT-19.08.vocab.txt" => 300 // phuonglh
      case _ =>
