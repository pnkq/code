...
/**
  ...
  */
object NewsIndexer {
  private val specialUnicodePattern = Pattern.compile("""[\u0000]+""")
  ...
  def run(jsonSource: String): Unit = {
    ...
    val news = if (useKafka) {
      ...
      val ns = urls.par.map(url => {
        ...
        if (content.length >= 500 && !content.contains("div") && !content.contains("class=") && !content.contains("script") && !specialUnicodePattern.matcher(content).find()) {
